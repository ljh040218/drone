...
초록색 마커를 탐지할 때, 오른쪽 가림막을 인식하여 링을 찾을 때 노이즈가 됩니다. 오른쪽 가림막이 나오지 않도록 드론의 회전각을 130 보다 줄이면 초록색 마커와 가림막을 제대로 찾지 못했습니다. 

이 때문에 중심점을 맞추지 못해 비행을 중앙을 잇는 직선으로 가지 못하는 문제가 발생했습니다.

따라서 저희는 2단 링 단계 코드를 수정했습니다. moveDroneToBlueCenter 함수를 사용하지 않고 2m 전진, 0.5m 왼쪽으로 이동 후 드론의 각도를 수정했습니다.
