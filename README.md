...
### 1-2단계에서 문제점 발생, 해결 방법
초록색 마커를 탐지할 때, 오른쪽 가림막을 인식하여 링을 찾을 때 노이즈가 됩니다. 오른쪽 가림막이 나오지 않도록 드론의 회전각을 줄이면 초록색 마커와 가림막을 제대로 찾지 못했습니다. 이 때문에 중심점을 맞추지 못해 비행을 중앙을 잇는 직선으로 가지 못하는 문제가 발생했습니다.

따라서 저희는 2단 링 단계 코드를 수정했습니다. moveDroneToBlueCenter 함수를 사용하지 않고
2m 전진, 0.5m 왼쪽으로 이동 후 드론의 각도를 수정했습니다.
아래는 대회장에서 드론이 가림막 내부 링과 초록색 마커를 인식하고 중점을 맞춰 이동한 모습입니다.

<p align="center">
<img src="https://github.com/user-attachments/assets/69d6f8bb-6d3a-4fc8-8fe2-cfd5e00f573c">
</p>


### 2-3단계에서 문제점 발생, 해결 방법
리허설 전 업데이트 된 대회장 이미지 속 보라색 마커를 탐지할 때 기존의 보라색 임계값을 사용하면 노이즈로 인해 탐지하지 못했습니다.
<p align="center">
<img src="https://github.com/JJeongAA/JJ_team/assets/168095384/492c8824-f8d6-48ee-8832-199c65afb419">
